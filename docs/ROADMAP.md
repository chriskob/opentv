...
free XMLTV guides, quality-variant grouping, favorites, categories, Movies + Series with
...
decoding is involved** — an archived program plays through the existing player exactly like a
movie. The work is reading the catch-up fields, building the right URL, and a guide gesture to
launch a past program.
...
| 5 | **Program reminders / notifications** | TiviMate, OTT Navigator | Recurrent request | Easy |
...
Honorable mentions: unified search across live/VOD/series, an "on now" strip, favorites-only
...
- **Local watch-history sync (device-to-device, no backend).** Viewella synced continue-watching
  ...
  pulls and merges newest-wins per item (resume positions, watched flags, favorites). No
...
- **What syncs:** resume positions, watched/finished flags, and favorites. Keyed by provider
...
- **Channels & categories** — hide, reorder, rename; manage favorites (the "Channel Manager"
  idea, done locally).
...
- **About** — version + in-app update check, GPL license, links to repo / issues / privacy, and
